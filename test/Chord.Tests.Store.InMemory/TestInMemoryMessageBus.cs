...
internal sealed class TestInMemoryMessageBus : IChordMessageBus
{
    private readonly ConcurrentDictionary<string, Func<ChordMessage, Task>> _handlers = new(StringComparer.OrdinalIgnoreCase);
    ...
    public Task SubscribeAsync(string queueName, Func<ChordMessage, Task> handler, CancellationToken cancellationToken)
    {
        ...
        cancellationToken.Register(() => _handlers.TryRemove(queueName, out _));
        return Task.CompletedTask;
    }
